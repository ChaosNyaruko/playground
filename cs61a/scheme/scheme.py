...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if scheme_length(formals) != scheme_length(vals):
            raise SchemeError('the numbers of formals and arguments do not match')
        child = Frame(self)
        for i in range(scheme_length(formals)):
            f = formals.first
            a = vals.first
            child.define(f, a)
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        env = None if not self.use_env else env
        try:
            if self.use_env:
...
def make_let_frame(bindings, env):
    ...
    while b is not nil:
        ...
        args = Pair(b.first.first, args)
        vals = Pair(scheme_eval(b.first.second.first, env), vals)
        b = b.second
    check_formals(args)
    let_frame = env.make_child_frame(args, vals)
    return let_frame
    # END PROBLEM 15
